...
RegisterNetEvent('ox_doorlock_toggleDoorlock')
AddEventHandler('ox_doorlock_toggleDoorlock', function(doorName)
	for _, door in pairs(doors) do
		if door.name == doorName then
			local doorData = getDoor(door)
			TriggerEvent('ox_doorlock:setState',doorData,0)
			return "yes"
		end
		return "no"
	end
