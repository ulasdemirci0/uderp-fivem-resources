...
RegisterNetEvent('brazzers-fakeplates:server:usePlate', function(vehicle, vehPlate, newPlate, hasKeys)
    ...
    if isFakePlateOnVehicle(vehPlate) then return TriggerClientEvent('QBCore:Notify', src,
        Lang:t("error.already_has_plate"), 'error') end
...
RegisterNetEvent('brazzers-fakeplates:server:removePlate', function(vehicle, vehPlate, hasKeys)
    ...
    if not isFakePlateOnVehicle(vehPlate) then return TriggerClientEvent('QBCore:Notify', src,
        Lang:t("error.does_not_have_fakeplate"), 'error') end
