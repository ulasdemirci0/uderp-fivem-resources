						...
						if Config.methLab.entry.requiredItem then
							QBCore.Functions.TriggerCallback("kaves_drugs:checkItem", function(output) 
								if output then
									...
									used = false
									QBCore.Functions.Notify("You don't have a lab card")
								end
							end, "labcard", 1)		
						else
							teleport(Config.methLab.entry.intcoord, Config.methLab.entry.intheading)
		...
		if dst < 3 and not cooking then
			...
			if dst < 2 and not cooking then
				...
				if IsControlJustPressed(0, 38) and not cooking then
					QBCore.Functions.TriggerCallback("kaves_drugs:checkItem", function(output) 
							if output then
								QBCore.Functions.TriggerCallback("kaves_drugs:checkItem2", function(output)  
									if output then
										...
										QBCore.Functions.Notify("You don't have a sodyum benzoat")
									end
								end, 'sacid', 1)
							else
								QBCore.Functions.Notify("You don't have a ammonia")
							end
					end, 'ammonia', 1)
				end 
			end
...
AddEventHandler("kaves_drugs:processMeth", function()
	...
	QBCore.Functions.Notify('you started the cooking process. Wait for it to finish %1')
	Citizen.Wait(10000)
	QBCore.Functions.Notify('you started the cooking process. Wait for it to finish %10')
	Citizen.Wait(10000)
	QBCore.Functions.Notify('you started the cooking process. Wait for it to finish %21')
	Citizen.Wait(10000)
	QBCore.Functions.Notify('you started the cooking process. Wait for it to finish %34')
	Citizen.Wait(10000)
	QBCore.Functions.Notify('you started the cooking process. Wait for it to finish %48')
	Citizen.Wait(10000)
	QBCore.Functions.Notify('you started the cooking process. Wait for it to finish %59')
	Citizen.Wait(10000)
	QBCore.Functions.Notify('you started the cooking process. Wait for it to finish %73')
	Citizen.Wait(10000)
	QBCore.Functions.Notify('you started the cooking process. Wait for it to finish %87')
	Citizen.Wait(10000)
	QBCore.Functions.Notify('you started the cooking process. Wait for it to finish %98')
	Citizen.Wait(5000)
	QBCore.Functions.Notify('The cooking process is over! go get it')
	while true do
		local playerPed = PlayerPedId()
		local playerCoord = GetEntityCoords(playerPed)
		local dst = #(playerCoord - Config.methLab.cookZone.coord)
		...
		if dst < 2 then
			if IsControlJustPressed(0, 38) then
				QBCore.Functions.TriggerCallback("kaves_drugs:checkCanCarryItem", function(output) 
					if output then
						...
						QBCore.Functions.Notify("You are too heavy!")
					end
				end, 'meth', count)
			end
		end
	...
	while true do
		...
		if dst < 5 and not process then
			...
			if dst < 3 then
				...
				if IsControlJustPressed(0, 38) then
					QBCore.Functions.TriggerCallback("kaves_drugs:checkItemAll", function(output, count) 
						if output then
							methCount = count
							...
							QBCore.Functions.Notify("You don't have a unpackaged meth")
						end
					end, 'meth')
				end
			end
