...
function setCoords(x, y) {
    $.post('https://ronin-dispatch/set-coords', JSON.stringify({
        x: x,
        y: y
    }));
}
...
$(document).ready(function() {
	window.addEventListener('message', function(event){
        ...
        if (event.data.action == "close") {
            $.post('https://ronin-dispatch/close', JSON.stringify({}));
            outKapa();
		}

        if (event.data.action == "close-callsign") {

            $.post('https://ronin-dispatch/close-callsign', JSON.stringify({}));
            outKapa();
		}
        ...
        $('.create-call').click(function() {
            $.post('https://ronin-dispatch/create_call', JSON.stringify({
                number: roninNumber
            }));
        })
        ...
        $('.dismiss-pingg').click(function() {
            ...
            createCall3.addClass('dismissed');
            $('.called-list').fadeOut();
            $.post('https://ronin-dispatch/dismiss_call-2', JSON.stringify({
                number: roninNumber
            }));
        })
    ...
    function vehiclesClick() {
        $('.vehicles').click(function() {
            ...
            if (value == 1) {
                ...
                removeItem()
                mesaiHold()
                $.post('https://ronin-dispatch/change-vehicle', JSON.stringify({
                    name: policeName,
                    icon: icon
                }));
                ...
                $.post('https://ronin-dispatch/table_ayril', JSON.stringify({
                    name: remove
                }));
    ...
    $('.dismiss-ping').click(function() {
        ...
        $.post('https://ronin-dispatch/dismiss_call-1', JSON.stringify({
            number: roninNumber
        }));
    })
    ...
    function assignButtons(){
        $('.assing-buttons').click(function() {
            $.post('https://ronin-dispatch/add_police-assign', JSON.stringify({
    ...
    $('.crate-dismiss').click(function() {
        ...
        $.post('https://ronin-dispatch/create_call-2', JSON.stringify({
            number: roninNumber
        }));
    })
    ...
    $('.remove-dismis-ping').click(function() {
        $('.dismiss-list').fadeOut();
        createCall4.remove();
        $.post('https://ronin-dispatch/remove-call', JSON.stringify({
            number: roninNumber
        }));
    })
    ...
    function cureungotunu() {
        ...
        $('.remove-unit').click(function() {
            ...
            $.post('https://ronin-dispatch/delete', JSON.stringify({
                name: player
            }));
            ...
            $('.mesai-personel').each(function() {
                if ($.trim($(this).text()) == text) {
                    ...
                    if (owner == player) {
                        ...
                        $.post('https://ronin-dispatch/under-police', JSON.stringify({
                            player: player,
                            owner: owner
                        }));
                    }
    ...
    function callCameRemove(){
        $('.call-came').dblclick(function(e){
            ...
            $('.remove-list .MuiPaper-rounded ul li').click(function(){
                $.post('https://ronin-dispatch/delete_police-assign', JSON.stringify({
                    ronin: callCame
                }));
            })
        })
    ...
    function save(){
        ...
        $.post('https://ronin-dispatch/save', JSON.stringify({
            html: save
        }));
