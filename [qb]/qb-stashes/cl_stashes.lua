    ...
    if Config.UseTarget then
        CreateThread(function()
            ...
            for k, _ in pairs(Config.Stashes) do
                exports['qb-target']:AddBoxZone(Config.Stashes[k].stashName,
                    vector3(Config.Stashes[k].coords.x, Config.Stashes[k].coords.y, Config.Stashes[k].coords.z), 1, 1,
                    {
                    ...
                    options = {
                        {
                            type = "client",
                            icon = "fas fa-box",
                            label = tostring(Config.Stashes[k].stashName) .. " Aç",
        ...
        while true do
            ...
            for k, v in pairs(Config.Stashes) do
                ...
                local dist = #
                    (
                    GetEntityCoords(ped) -
                        vector3(Config.Stashes[k].coords.x, Config.Stashes[k].coords.y, Config.Stashes[k].coords.z))
                if dist <= 2.0 then
                    wait   = 5
                    inZone = true
...
RegisterNetEvent('qb-business:client:openStash', function(currentstash, _)
    ...
    if Config.PoliceOpen then
        if PlayerJob == "police" and PlayerData.job.grade.level >= 3 then
            canOpen = true
        end
    end
    ...
    if Config.Stashes[currentstash].canAnyoneOpen then
        TriggerServerEvent("inventory:server:OpenInventory", "stash", Config.Stashes[currentstash].stashName,
            { maxweight = Config.Stashes[currentstash].stashSize, slots = Config.Stashes[currentstash].stashSlots })
    ...
    elseif canOpen then
        ...
        QBCore.Functions.Notify('Bu depoya ulaşamazsın!', 'error')
    end
